      ...
      <span className="block w-fit items-center flex justify-between text-base font-semibold leading-6 text-gray-900 w-full">
        ...
        {!mobileMenuOpen ? <ButtonCircle isDropdownOpen={isDropdownOpen} /> :
          <div className="block w-5 top-1/2 z-30 transform">
            <span aria-hidden="true" className={`block absolute h-[2px] w-[14px]  bg-gray-500 transform transition duration-300 ease-in-out ${isDropdownOpen ? 'rotate-45' : ' rotate-90'}`}></span>
            <span aria-hidden="true" className={`block absolute  h-[2px] w-[14px] bg-gray-500  transform  transition duration-300 ease-in-out ${isDropdownOpen ? '-rotate-45' : ' -rotate-180'}`}></span></div>}
      </span>
