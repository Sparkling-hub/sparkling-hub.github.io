...
const MyMultipleSelect: React.FC<IMultiSelect> = ({ dataJobs, uniqueIds, location, placeholder, handleLocationChange, deleteItem }) => {
	...
	return (
		...
		<div className={`my-multiple-select ${isActive ? 'active' : ''} m-4`} >
			...
			<div className={`my-multiple-select-container flex ${isActive ? 'rounded-t-lg' : 'rounded-lg'}  relative bg-color-primary-medium border-bg-color-primary-medium`} onClick={handleClick} onKeyDown={handleClick}   tabIndex={0}>
				...
				<div className={`${location.length > 0 ? 'w-1/5' : ''} px-4`} >
					...
					/>
					<div className="absolute top-19 h-max w-full left-0 z-10 list flex flex-wrap items-center p-4  rounded-b-lg border-t-[1px] border-slate-200 bg-color-primary-medium border-bg-color-primary-medium" onClick={handleClickDropDown} onKeyDown={handleClickDropDown}   tabIndex={0}>
