...
const MySingleSelect: React.FC<ISingleSelect> = ({ dataJobs, uniqueExp, position, placeholder, handleExpChange }) => {
	...
	return (
		...
		<div className={`my-single-select ${isActive ? 'active' : ''} m-4`} >
			...
			<div className={`my-single-select-container flex ${isActive ? 'rounded-t-lg' : 'rounded-lg'}  relative bg-color-primary-medium border-bg-color-primary-medium`} onClick={handleClick} onKeyDown={handleClick}   tabIndex={0}>
